...
PASS=$(cat /etc/balance/pass)

#echo "$host: I am host"
#echo "${TARGET}: I am TARGET"
#echo "$mode: I am mode"
#echo "$EXCLUDE.$mode: I am EXCLUDE.mode"


rsync -avv --ignore-errors --whole-file --rsh="sshpass -p $PASS ssh"  \
--exclude-from="$EXCLUDE.$mode" / ${host}:${TARGET}


# Test
#rsync -avv --delete --ignore-errors --whole-file --rsh="sshpass -p $PASS ssh" / ${host}:${TARGET}

# Example command
#rsync --archive --rsh=ssh --delete --ignore-errors --whole-file \
#--exclude-from="$EXCLUDE.$mode" / ${host}:${TARGET}

#Example from my previous product video lessons
